...
class User {
  ...
  {
    const userRoleQry = `SELECT ef_action as action, ef_component as component, ef_display_name as display_name
                        FROM epo_feature, epo_feature_role_map
                        WHERE 
                        ef_inmenu = '1'
                        AND efrm_role_id = ${roleid}
    ...
    userRoleResult.forEach((result) => {
      if(result['action'] != null && result['action'] != '')
      {
        ...
        const routes = { path: featureUrl, component: component, url: url, title: display_name};
      ...
      const insertUserQry = `INSERT INTO epo_login
        ...
        const insertUserInfoQry = `INSERT INTO epo_user_info
                                    ...
                                    ( 
                                      ...
                                      ${file ? sqlstring.escape(`/images/profilepic/`+requestData.empid + path.extname(file.originalname)) : `''`}
                                    )`;
  ...
  static updateUser(requestData,file)
  {
    ...
    if(updateUserRst)
    {
      const imageUpdate = file ? 'eui_image_path = '+sqlstring.escape('/images/profilepic/'+requestData.empid + path.extname(file.originalname))+',' : '';
